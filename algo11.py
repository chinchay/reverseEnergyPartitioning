...
if (rank == 0):
    ...
    #
    for i in range(size):
        comm.send(continuar , dest=i , tag =11)
    #
    #
#
#
if  (rank != 0):
    continuar = comm.recv(source=0 , tag =11)
#
...
while ( continuar and (i < iMax) ): # iMax allows to force stopping.
    ...
    if (rank == 0):
        listCfgs = []
        lCfgs, alpha, log_idos, log_sum_idos = mc.getValuesAfterParallelWork2(I1, I2, log_idos, log_sum_idos, lCfgs)
        #
    ...
    if doLastStep:
    ######## Parallel work: ####################################################
        I1, I2, lCfgs = mc.randomMCmovesParallel2(Emin, E[m - 2], E[m - 1], Einfty, nSteps, e, Xcopy, L, structEq, Xeq, forceMatrix)
        I1 = comm.gather(I1, root =0)
        I2 = comm.gather(I2, root =0)
        lCfgs = comm.gather(lCfgs, root =0)
        #
