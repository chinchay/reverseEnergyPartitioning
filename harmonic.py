...
#
#     eig = getRidZeros(eig) # paper: "zeros do not contribute to DOS" <<< still wondering why so many zeros???????????
#     # eig is now an array, not a numpy array!
#     return productSqE(eig)
...
# coef(N, V)
#
# eig = LA.eigvalsh( getForceMatrix() )
# eig = getRidZeros(eig)
# m1 = productSqE(eig)
# m1
#
...
def getUnitCell():
    ...
    # fcc:
    alat =  [sq2s2 * (ex + ey), sq2s2 * (ey + ez), sq2s2 * (ex + ez)]
    r0 = getVector(0, 0, 0, alat)
    r1 = getVector(1, 0, 0, alat)
    r2 = getVector(0, 1, 0, alat)
    r3 = getVector(0, 0, 1, alat)
    r4 = getVector(1, 1, 0, alat)
    r5 = getVector(0, 1, 1, alat)
    r6 = getVector(1, 0, 1, alat)
    r7 = getVector(1, 1, 1, alat)
...
def getSuperCell():
    ...
    lattice = Lattice.from_parameters(a=la, b=lb, c=lc, alpha=90,
                                      beta=90, gamma=90)
...
def getNeighborsOfEachAtom(radio):

    return neighborsIndxOfEachAtom
#
...
def loopFcc(positionAtomsInput, isFirstTime):
    import potential
    from potential import VLJ as V
