...
def getRandomWalk(L): # L: amplitud
    ...
    return L * (random.random() - 0.5) # `0.5`:for centering around 0.
...
def mc_move(i, E_old, X_old, Xeq, forceMatrix, E_m_minus_1, E_m_minus_2, L, structEq):
    import copy
    import harmonic as ha
    ...
    E_new, energyPerParticle = getEnergyConfig(X_new, structEq)
    P_old2new = getProbTransition(E_new, E_old, E_m_minus_1, E_m_minus_2)
    hasMoved = False

    # if ( P_old2new >= random() ):
    if ( P_old2new >= random.random() ):
        hasMoved = True
        ...
        if (c % 1000 == 0):
            ...
            out = "ovitoPlots/rHyperEHarmonic" + str(c) + ".txt"
...
def randomMCmoves(Eminimo, E_mMinus2, E_mMinus1, E_m,\
                  nSteps, e, X, log_idos, log_sum_idos, L, structEq, Xeq, forceMatrix):
    import copy
    import harmonic as ha
...
# MC sampling with weight w(E). Collect quantities for the next subdivision m+1.
################################################################################
def randomMCmovesParallel(Eminimo, E_mMinus2, E_mMinus1, E_m,\
                  nSteps, e, X, L, structEq, Xeq,\
                  forceMatrix, output):
    import copy
    import harmonic as ha
...
def getValuesAfterParallelWork(results, log_idos, log_sum_idos):
    sizeResults = len(results)
    I1 = sum( [results[i][0] for i in range(sizeResults)] )
    I2 = sum( [results[i][1] for i in range(sizeResults)] )
    lCfgs = []
    ...
    return lCfgs, alpha, log_idos, log_sum_idos
#
def getValuesAfterParallelWork2(I1, I2, log_idos, log_sum_idos, lCfgs):
    sizeResults = len(results)
    I1 = sum( [I1[i] for i in range(I1)] )
    I2 = sum( [I2[i] for i in range(I2)] )
    lCfgsFlatten = []
    for i in range(len(lCfgs)):
        for j in range(len(lCfgs[i])):
            lCfgsFlatten.append(lCfgs[i][j])
    #
    ...
    return lCfgsFlatten, alpha, log_idos, log_sum_idos
#
...
def select_start_config(lCfgs, E_mMinus1, Em, nthMin):
    ...
    if nthMin == 1:
        ...
        #
        if (len(esaved) == 0):
            esaved = [ lCfgs[i][0] for i in range(len(lCfgs)) ]
            Xsaved = [ lCfgs[i][1] for i in range(len(lCfgs)) ]
        #
        minE = max(esaved)
        idx = esaved.index(minE)
        Xtemp = Xsaved[idx]
