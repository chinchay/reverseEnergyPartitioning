...
def getEnergyConfig(X):
    ...
    return cf.getLJenergy(X, sigma, epsilon)
#
################################################################################
...
def randomMCmoves(Eminimo, E_mMinus1, E_m, E_mplus1,\
    ...
    # acceptance toward the range 25 -35 percent. If the acceptance rate during a
    ...
    while( (not belongs(ratioAcceptances, ratioMin, ratioMax) and\
        ...
        # I1 = c*Int_Em-1^Em Omega(E)dE;  I2 = c*Int_Emin^Em-1 Omega(E)dE, `c` is some constant.
        ...
        for i in range(nSteps):
            ...
            # it has no sense to add the same configuration (not moved) to Omega:
            if hasMoved:
                acceptances += 1 # accepted move.
                if belongs(e, Eminimo, E_m): # (Emin <= e <= E[m - 1]): #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                    I_EminToEm += 1  # un gol adicional para Int_Emin^Em-1 Omega(E)dE
                elif belongs(e, E_m, E_mplus1): # (E[m - 1] <= e <= Em):
                    I_EmToEmplus1 += 1 # un gol adicional para Int_Em-1^Em Omega(E)dE
                elif E_mplus1 < e:
                    I_Emplus1ToInfty += 1
                #
    ...
    # calculate alpha_m, log_idos:
    alpha = I_EmToEmplus1 / I_EminToEm # MonteCarlo: ~cociente de goles Int_Em-1^Em  / Int_Emin^Em-1
    assert(alpha != 0)
